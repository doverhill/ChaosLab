...
pub const BOGUS_AUTO_BOTH_MIXED_ARGUMENTS_ENUM_OBJECT_ID: usize = 10;

pub enum BothMixedArgumentsEnum {
    Window(crate::Window),
    Button(crate::Button)
}

impl ChannelObject for BothMixedArgumentsEnum {
    ...
    unsafe fn from_channel(pointer: *const u8) -> Self {
        ...
        match kind {
            ...
            _ => {
                panic!("Received unexpected value for BothMixedArgumentsEnum");
            }
        }
    }
}

pub struct BothMixedMixedArgumentsIterator {
    channel_reference: Arc<Mutex<Channel>>,
    index: usize,
    item_count: usize
}

impl BothMixedMixedArgumentsIterator {
    pub fn new(channel_reference: Arc<Mutex<Channel>>) -> Self {
        ...
        BothMixedMixedArgumentsIterator {
...
impl Iterator for BothMixedMixedArgumentsIterator {
    type Item = crate::BothMixedArgumentsEnum;

    fn next(&mut self) -> Option<Self::Item> {
        if self.index < self.item_count {
            let channel = self.channel_reference.lock().unwrap();
            self.index += 1;
            match channel.get_object::<crate::BothMixedArgumentsEnum>(self.index - 1, crate::BOGUS_AUTO_BOTH_MIXED_ARGUMENTS_ENUM_OBJECT_ID) {
                Ok(object) => {
                    Some(object)
                },
                Err(_) => {
