...
pub fn draw_text(mut client: RefMut<Client>, font: &Font, text: &String) {
    ...
    let foreground_color = convert_color_console_to_sdl(Color { alpha: 255, red: 255, green: 255, blue: 255 });
    let background_color = convert_color_console_to_sdl(Color { alpha: 255, red: 0, green: 128, blue: 255 });
    let surface = rendering.shaded(foreground_color, background_color).unwrap();
    surface.blit(None, &mut client.surface, Rect::new(0, 0, 100, 100)).unwrap();
}
